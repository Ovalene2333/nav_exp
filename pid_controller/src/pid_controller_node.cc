...
namespace pid_controller
{
    ...
    geometry_msgs::Twist PIDController::calculateVelocity(const geometry_msgs::PoseStamped &current_pose, nav_msgs::Path &path)
    {
        ...
        ROS_INFO("ex:%.3f ey:%.3f", error_x, error_y);
        ...
        cmd_vel.linear.x = p_term_x + i_term_x + d_term_x;
        cmd_vel.linear.y = p_term_y + i_term_y + d_term_y;

        // 限制速度
        cmd_vel.linear.x = std::max(-max_vel_x_, std::min(max_vel_x_, cmd_vel.linear.x)) / physic_vmax_rate_;
        cmd_vel.linear.y = std::max(-max_vel_y_, std::min(max_vel_y_, cmd_vel.linear.y)) / physic_vmax_rate_;
